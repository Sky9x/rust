...
///
/// exampling:
/// say you wrap this in a `Foo`:
/// public API exposes `Pin<&mut Foo>`, which does not invalidate any `*const T`
/// or `*mut T` to the `T` in the `UnsafePinned<T>`. yay everyone happy
///
// FIXME(unsafe_pinned): actually block niches (either by bringing back #[repr(no_niche)]
//  or by doing it manually on the lang item)
...
/// When this type is used, that almost certainly means safe APIs need to use
/// pinning to avoid the aliases from becoming invalidated. Therefore let's mark
/// this as `!Unpin`. You can always opt back in to `Unpin` with an `impl`
...
impl<T> UnsafePinned<T> {
    /// Constructs a new instance of `UnsafePinned` which will wrap the
    /// specified value.
    ///
    /// All access to the inner value through `&UnsafePinned<T>` or
    /// `&mut UnsafePinned<T>` or `Pin<&mut UnsafePinned<T>>` requires `unsafe`
    /// code.
    #[inline(always)]
    #[must_use]
    #[unstable(feature = "unsafe_pinned", issue = "125735")]
...
impl<T: ?Sized> UnsafePinned<T> {
    ...
    pub const fn get_mut_pinned(self: Pin<&mut Self>) -> *mut T {
        ...
        unsafe { self.get_unchecked_mut() as *mut Self as *mut T }
    }
